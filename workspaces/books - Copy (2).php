...
<html>
	<head>
		...
		//	To determine which row is clicked on, and therefore to determine which Book to then subsequently load. 
			$(document).ready(function(){
				$("#bookList td").click(function() {     
					...
					loadMe = $('td:first', $(this).parents('tr')).text();
					$("#result").html(loadMe);   
					$("#b_id").val(loadMe);
					
					$("#bookForm").submit(function(eventObj) {
		...
		<script>
		    //binds the formOnSubmit method below to run as part of your form's onsubmit method
			$(function(){
				$('#bookForm').submit(formOnSubmit);
