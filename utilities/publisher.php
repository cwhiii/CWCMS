...
class Publisher{
		...
		function publish($pid){
			$this->getRaw($pid);
			$this->echoAll();
			$_SESSION['FINAL_URL'] = $_SESSION['FULLPATH'] . $_SESSION['NAME'] . $_SESSION['pageExtention'];
			$this->c3Log("Final URL:".$_SESSION['FINAL_URL']);
			$this->buildPage();
			}
		...
		function buildPage(){
			...
			if (file_put_contents($_SESSION['FINAL_URL'], $_SESSION['COOKED_CONTENT']) !== false) {
				$this->c3Log("File created: " . basename($_SESSION['FINAL_URL']) ."." );
				} 
			else {
				$this->c3Log("Cannot create file (" . basename($_SESSION['FINAL_URL']) );
				}
			}
	}
?>
